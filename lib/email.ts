...
export async function sendResultEmail({
  ...
  const { data, error } = await resend().emails.send({
    ...
    subject,
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #030A14; color: #E8F4FF;">
        <h1 style="color: #00FFFF; font-size: 24px; margin-bottom: 20px;">${escapeHtml(siteName)}</h1>
        ...
        ${resultHtml}
        <hr style="border: 1px solid #0A1622; margin: 30px 0;" />
        <p style="font-size: 12px; color: #556A7A;">
